...
module.exports.api = {
  ...
  updateTeamById: async (teamId) => {
    ...
    try {
      ...
      let playerList = await Promise.all(currentTeam.players.map(async player => {
  ...
  removePlayerFromTeam: async (teamId, playerId, action) => {
    ...
    try {
      ...
      const redeem =
        action === "redeem"
          ? computeRedeem(redeemType, currentPlayer.price)
          : 0;
      ...
      let result = await teams.updateOne(
        { _id: new ObjectId(teamId) },
        { $pull: { players: { id: playerId } }, $inc: { redeemAmount: inc } }
      );
